...
+++
Agent平台算法需求落地	
1、响应 DeepSearch 标注接入需求，支持模型调试环境
2、探索 DeepResearch-Browser Use 前沿算法需求
3、基于 OWL 框架跑通 GAIA 数据集，搭建多模型评测链路
4、建设 Agent 数据合成框架的工具系统，提供 OWL 工具链支持

微信公众号优化交互体验与风控
1、开发反问链路自定义提示词模版，增强对话灵活性
2、设计 Emoji 表情数量控制机制，规范回复风格
3、优化回复分段逻辑算法，改善长文本阅读体验
4、针对高危行业场景，植入特殊提示词防御策略

AI搭子核心链路构建与技术选型：主导 AI 搭子一期核心对话链路的工作流工程落地
模型训练压测与系统稳定性建设：
1、开展 Agent 强化学习框架的专项测试支持，保障训练环境稳定性
2、针对 Coding Agent (OpenHands) 执行高并发压测，验证系统极限性能。
3、实施 AgentFlow 接口的幂等性改造，提升系统健壮性。

企业微信混元助手客服搭建：
基于云函数，搭建接入企业微信的混元助手智能客服系统
...
支持微信公众号与元器的合作:
搭建微信公众号后台，负责支持公众号主创建公众号智能体、并在微信开放平台上调试智能体效果，将聊天链路从Competition切换到AssistantAPI
...
混元官网:
1、上线混元agent开发平台，支持mcp，负责开发mcp server，在混元官网上架搜索、金融、生活效率、论文、同程旅游等官方插件
2、对接QLabel平台支持DeepSearch标注需求
...
3、生图链路耗时优化
1. 支持对话式图文多模态能力
2. 支持为上传/生成的图片自动生成描述，可以复制描述后二次编辑，生成新的图
3. 支持用户输入图片和文本描述，生成新的图片
...
系统安全性整改：
1、MySQL deleted_at字段为NULL导致唯一索引失效问题修复
2、配置规范
3、降级上报告警

数据飞轮：
文生图在部分领域由于数据样本量不足、质量不佳等问题，导致文生图效果不佳。需要数据飞轮项目，来分析、解决数据存在的质和量的问题，提升文生图效果。
1.针对badcase完成8月至今近2千个prompt共1.2万张图片的现状分析、训练集检索、主题词抽取，补充了飞轮中113万训练集，V1.2版本单轮评测解决了47%的badcase
2.将飞轮的badcase收集 - 分析 - 检索 - 数据标注 - 加入精品库 - 效果反馈等各环节流程进行线上化
3.将qlabel、验证码等标注流程实现自动化，节省人力

通用推荐：
通用推荐 检索引擎重构
负责检索引擎重构方案设计并参与开发。
调研业界检索引擎方案以及es、ef等多种组件能力，结合项目实际情况，选型es8，重构包括icf、tag、hot、random等多路召回
上线过载保护功能
7月完成多租户改造，支持接入零售电商客户
完成数据集模块、倒排服务、特征中心等模块支持多租户改造，并接入cos\ur\lining等客户
